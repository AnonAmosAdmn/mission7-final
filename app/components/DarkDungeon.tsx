...
function loadBase64Image(base64: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    const img = new (globalThis.Image as typeof HTMLImageElement)();
...
class Game {
  ...
  draw_ui(ctx: CanvasRenderingContext2D) {
    ...
    // SWAPPED: Time now appears where HP was
    const minutes = Math.floor(this.timeRemaining / 60000);
    const seconds = Math.floor((this.timeRemaining % 60000) / 1000);
    const timeText = `${minutes}:${seconds.toString().padStart(2, '0')}`;
    ctx.fillStyle = this.timeRemaining < 30000 ? '#FF0000' : WHITE;
    ctx.fillText(`Time: ${timeText}`, centerX - 100, textY);
    
    // SWAPPED: HP now appears where Time was
    const healthPercentage = this.player.health / this.player.maxHealth;
    ctx.fillText(`HP: ${this.player.health}/${this.player.maxHealth}`, centerX, textY);
    ctx.fillStyle = '#333';
    ctx.fillRect(centerX - 50, textY + 10, 100, 10);
    ctx.fillStyle = healthPercentage > 0.6 ? '#00FF00' : 
                    healthPercentage > 0.3 ? '#FFFF00' : '#FF0000';
    ctx.fillRect(centerX - 50, textY + 10, 100 * healthPercentage, 10);
